...
class Music(ParentCog):
    ...
    async def join(self, ctx:commands.Context):
        if ctx.author.voice is not None and ctx.voice_client is not None and ctx.author.voice.channel.id == ctx.voice_client.channel.id:
    ...
    async def leave(self, ctx:commands.Context):
        if not self._bot_in_user_channel(ctx):
            ...
            if ctx.voice_client.is_playing():
                self._stop_without_queue_next(ctx)
    ...
    def _bot_in_user_channel(self, ctx:commands.Context) -> bool:
        return ctx.author.voice is not None and ctx.voice_client is not None and ctx.author.voice.channel.id == ctx.voice_client.channel.id


    def _play(self, ctx, player: YTDLSource):
        ctx.voice_client.play(player, after = lambda e: self._after(ctx, e))
        self.currently_playing = player.title


    def _stop_without_queue_next(self, ctx):
        ctx.voice_client.pause()
